...
public class UnifiedWordLoader extends AbstractWordLoader {
    ...
    private void loadStyleAndTimeFiles(WordBank wordBank, String basePath) throws IOException {
        ...
        for (String style : styleKeys) {
            loadSingleFile(wordBank, basePath + WordBankConstants.STYLE_PREFIX + style + WordBankConstants.FILE_EXTENSION);
        }

        // 加载时间文件
        String[] timeKeys = {"morning", "noon", "night", "weekend", "normal"};
        for (String timeKey : timeKeys) {
            loadSingleFile(wordBank, basePath + WordBankConstants.TIME_PREFIX + timeKey + WordBankConstants.FILE_EXTENSION);
        }
    }
